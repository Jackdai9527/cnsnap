...
export async function generateMetadata({ params }: BlogCategoryPageProps): Promise<Metadata> {
  ...
  const locale = await getRequestSeoLocale() || "en";
  ...
  return createBlogTaxonomyMetadata({
    ...
    pathname: `/blog/category/${result.category.localizedSlug || result.category.slug}`
  });
}
...
export default async function BlogCategoryPage({ params }: BlogCategoryPageProps) {
  ...
  const structuredData = await createBlogCategoryStructuredData(result.category);
      ...
      <section className="site-container py-4">
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {result.articles.map((article) => (
            <SeoLocaleLink key={article.id} href={`/blog/${article.slug}`} className="rounded-[26px] border border-[#dfe7f1] bg-white p-5 shadow-[0_16px_36px_rgba(15,23,42,0.05)] transition hover:border-[#0a83ff]">
